...
export function exportToExcel(
  ...
  const eventData = [
    ...
    ['TOTALS'],
    [],
    ['Net Sales', totals.totalNetSales],
    ['Cash Sales', totals.totalCashSales],
    ['CC Sales', totals.totalCcSales],
    ['CC Gratuity', totals.totalCcGratuity],
    ['Cash Gratuity', totals.totalCashGratuity],
    ['Total Points', totals.totalPoints],
  ]

  const eventWs = XLSX.utils.aoa_to_sheet(eventData)
  XLSX.utils.book_append_sheet(wb, eventWs, 'General Information')
  ...
  const salesData = [
    ...
    [],
    ['TOTALS', '', totals.totalNetSales, totals.totalCashSales, totals.totalCcSales, totals.totalCcGratuity, totals.totalCashGratuity, totals.totalPoints],
  ]

  const salesWs = XLSX.utils.aoa_to_sheet(salesData)
  XLSX.utils.book_append_sheet(wb, salesWs, 'Sales Details')

  // Gratuity Distribution Sheet
  const tipData = [
    ['GRATUITY DISTRIBUTION'],
    [],
    ['EMPLOYEE', 'CC GRATUITY', 'CASH GRATUITY', 'TOTAL GRATUITY', 'POINTS', '% OF TOTAL'],
    ...tipDistribution.map((row) => {
      const employeeData = sales.find(s => s.employee === row.employee)
      const points = employeeData?.points || 0
      const percentage = totals.totalGratuity > 0 ? (row.tips / totals.totalGratuity) * 100 : 0
      return [
        row.employee,
        row.ccGratuity || 0,
        row.cashGratuity || 0,
  ...
  XLSX.utils.book_append_sheet(wb, tipWs, 'Gratuity Distribution')

  // Generate filename with timestamp
  const date = eventInfo.date || new Date().toISOString().split('T')[0]
  const eventName = eventInfo.eventName || 'EVENT'
  const filename = `${eventName}_${date}_SalesReport.xlsx`
